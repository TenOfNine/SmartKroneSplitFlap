...
| 0.26 | 13.09.2026 | **Master-Firmware (T12/T13, ESP32-C3-Port + Web-UI) am Gerät bestätigt** — auf einem ESP32-C3 geflasht, WLAN und Web-UI liefen (Betreiber, vor dem Einbau in die Master-PCB). Das RS-485-Zusammenspiel mit den Daughter Cards bleibt offen, bis die Master-PCB aufgebaut ist (TP8485E-Transceiver, CHAIN-Pegelwandler nur dort vorhanden). Keine Code-Änderung, reine Statuskorrektur in Backlog/Projektstand. |
| 0.25 | 12.09.2026 | Projekt-Logo (`docs/logo.svg`): Fallblatt-Kachel mit Buchstabe „K", Gehäuse `#0b0c0e`/`#131519`, Ziffer im UI-Amber `#f2b03d`. Als Favicon (Inline-SVG-Data-URI) in der Master-Web-UI (`INDEX_HTML`) und im Browser-Flasher (`prebuilt/index.html`) eingebunden, zusätzlich in der README. Rein kosmetisch, keine Funktionsänderung. |
