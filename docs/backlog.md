...
- **OTA-Update aus dem Browser** (`POST /api/update`, `Update`-Bibliothek):
  App-Image hochladen → zweite App-Partition → Neustart.
  `tools/build_master_firmware.py` legt `factory.bin` (USB) und das App-Image
  (OTA) nach `prebuilt/`. `ArduinoOTA` entfernt (passwortloser espota-Port); der
  `ota_enabled`-Schalter gated `/api/update`. *(Seit T15: signierter Container
  `krone-master-esp32c3.kota` statt der rohen `.bin`.)*
