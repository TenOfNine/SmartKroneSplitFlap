...
| Master-Sicherheit (T15) | Signiertes Browser-OTA (`.kota`-Container, ECDSA P-256 über SHA-256, Public Key einkompiliert, `lib/otaverify` host-getestet, `src/ota_sign.cpp` mbedTLS, `tools/ota_keys.py`, `docs/firmware-signing.md`). Zugriffsschutz: `guard()`-Wrapper mit Herkunftsfilter `net_scope` (Vorgabe RFC1918) + optionaler HTTP-Basic-Auth. Repo-Hygiene: E-Mail aus Doku, Messfotos ohne EXIF + verkleinert, `.gitignore`. `pio test -e native` 50/50, Flash ~1010 KB. Spez. v0.16. Am Gerät ungetestet. | PR #8/#9 |
| Browser-UPDI-Flasher (T16) | Zweiter Tab „Daughter Card" im Web-Flasher: `firmware/master/prebuilt/updi.js` (SerialUPDI auf Web Serial, Geräte-ID-Prüfung + Chip-Erase + Page-Write + Verify), `tools/build_module_firmware.py` → `firmware/module/prebuilt/*.hex`, `pages.yml` stellt Publish-Ordner aus beiden `prebuilt/` zusammen. README-Ausblick: Flashen über den Bus (spätere Layout-Revision). **Werksflash am Gerät verifiziert (10.09.2026)**, RSD-Bug im Seiten-Schreiben behoben (`59710f8`). Spez. v0.17/v0.22. | PR #10, `1a600af`, `59710f8` |
...
**Hardware Master (Rev. 0.2, T20 — PR #14/#15 gemergt)**
- Symbolprüfung + PCB vom Betreiber freigegeben, DRC 0/0/0.
- **Master-PCB bei JLCPCB bestellt (10.09.2026)** — Gerber/BOM/CPL in
  `hardware/master/manufacturing/`.
- Offen: Ringkerntrafo 230 V → 2 × 18 V, 50–60 VA, EN 61558; primär träge
  Sicherung + NTC.
