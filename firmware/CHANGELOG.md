...
| 1.11 | `afe15e8` | Mobile Web-UI: Marke/Titel blieb beim horizontalen Scrollen der Navigationsleiste unter 820 px Breite nicht sichtbar (`.side` scrollte als ein Block samt `.brand`) — Marke jetzt fest, nur die Tab-Leiste scrollt. Modul-Version mitgezogen (kein Funktionsunterschied, nur Versionsgleichstand) |
| 1.12 | `d73b2dd` | Fix: `g_bus.mod[].ver_known` wurde nach der ersten erfolgreichen `CMD_GET_VERSION`-Antwort nie wieder zurückgesetzt — der reguläre Versions-Poll fragt ein "bekanntes" Modul nie erneut ab, daher zeigte die UI nach einem erfolgreichen Bus-Update weiter die alte Version an, obwohl `lib/moduleupdate` die neue Version bereits selbst per `GET_VERSION` bestätigt hatte. `ver_known` wird jetzt zurückgesetzt, sobald `moduleupdate` eine Adresse abschließt (Erfolg oder Fehlschlag), sodass der reguläre Poll sie neu lernt. Modul-Version mitgezogen (kein Funktionsunterschied, nur Versionsgleichstand). |
| 1.13 | `3a72acf` | Synchrones LED-Blinken: neuer Broadcast `CMD_LED_SYNC` (0x41, `lib/protocol`), den der Master alle ~1 s sendet (`busmaster_led_sync()`); Module (und der Master selbst über `g_bus.led_sync_ms`) nullen darauf ihre Blinkphase, statt seit dem jeweils eigenen Boot-Zeitpunkt zu zählen — alle Status-LEDs blinken jetzt im gleichen Takt. |
| 1.14 | `15d4065` | Fix: Eine Karte, die mitten in `FW_DATA`/`FW_END` hängen bleibt (App bereits ungültig markiert, Bootloader wartet korrekt auf neues `FW_BEGIN`), beantwortet `CMD_GET_STATUS` nicht mehr und gilt als offline — „Alle aktualisieren"/„Veraltete aktualisieren" filtern aber beide auf online, die Karte war über den Bus nie wieder erreichbar. Neuer Button „Offline erneut versuchen" + `handle_module_update()` lässt explizit angefragte Adressen immer zu, unabhängig vom Online-Status. Am Gerät gefunden (21.09.2026, Modul 2 nach abgebrochenem Update), Fix noch nicht am Gerät verifiziert. |
| 1.15 | *(dieser Commit)* | Fix: Web-UI-Moduswahl „Text" sprang beim nächsten Status-Poll (alle 2,5 s) zurück auf den vorherigen Modus, bevor überhaupt etwas eingetippt werden konnte — der Klick auf „Text" sendet bewusst noch keinen `/api/mode` (erst „Senden"), der Poll glich die Anzeige aber sofort wieder an den unveränderten Server-Modus an. Neues `textPending`-Flag unterdrückt den Abgleich, solange die Texteingabe offen und unbestätigt ist. Modul-Version mitgezogen (kein Funktionsunterschied, nur Versionsgleichstand). |
