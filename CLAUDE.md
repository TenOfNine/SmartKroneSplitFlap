...
- Master- und Modul-Firmware teilen sich eine Version (`MAJOR.MINOR`, siehe `firmware/CHANGELOG.md`). Bei jeder ausgelieferten Firmware-Änderung: `FW_VERSION_MINOR` (Master, `main.cpp`) und `APP_VERSION_MINOR` (Modul, `board.h`) gemeinsam erhöhen und eine Zeile in `firmware/CHANGELOG.md` anhängen. `MAJOR` nur bei großen Änderungen (Protokollbruch, Architekturwechsel, Übergang Inbetriebnahme → Dauerbetrieb).
...
- **`tools/gen_firmware_memory_chart.py` ebenfalls bei jeder Firmware-Änderung neu ausführen** (→ `firmware/speicherprofil.svg`, eingebunden in `firmware/README.md`) — Speicherverbrauch nach Bereich für Master (ESP32-C3) und Modul (ATtiny1616), gemessen am gelinkten Firmware-Abbild. Sonst zeigt die Grafik unbemerkt einen veralteten Stand.
