...
def main() -> int:
    ...
    (OUT / "README.md").write_text(
        ...
        "- **Web-UI:** *Einstellungen > System > Firmware aktualisieren* -> "
        "`krone-master-esp32c3.ota.bin` hochladen. Kein Toolchain, jeder Browser. "
        "Bei Fehler bleibt die alte Firmware aktiv. Einstellungen (NVS) bleiben erhalten.\n"
        "- **PlatformIO:** `pio run -e esp32c3 -t upload -d firmware/master` "
        "mit `--upload-port krone_anzeige.local` (ArduinoOTA, im Netz aktiv).\n\n"
